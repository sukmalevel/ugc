...
async function runStoryboardWorkflow() {
    ...
    if (!productDescEl) { showToast("Elemen deskripsi produk tidak ditemukan!", "error"); return; }
...
1. VO di setiap scene bersifat OPSIONAL. AI bebas menentukan scene mana saja yang memerlukan VO dan mana yang dibiarkan tanpa VO (kosong/diisi "" di JSON). 
2. TOTAL seluruh kata Voiceover (VO) dalam SATU STORYBOARD secara kumulatif WAJIB BERKISAR ANTARA MINIMAL 15 KATA DAN MAKSIMAL 20 KATA. 
3. Untuk scene yang TIDAK membutuhkan VO, isikan nilai properti "voiceover" dengan string kosong ("") pada skema JSON.
ATURAN EMAS KONTINUITAS & CERITA BERSAMBUNG:
Jika jumlah storyboard yang dipilih oleh user lebih dari 1, semua storyboard tersebut wajib dirancang sebagai satu cerita tunggal yang berjalan maju secara kronologis dan berkesinambungan.
Pastikan elemen berikut konsisten sepanjang rangkaian storyboard: Karakter/Talent yang sama, Produk yang sama persis, Lokasi dan setting lingkungan yang sama, Timeline berjalan maju tanpa reset.
ATURAN KETAT PENEMPATAN CALL TO ACTION (CTA) & PENUTUP:
1. JIKA USER MEMILIH HANYA 1 STORYBOARD: CTA dan penutup final BOLEH diletakkan di storyboard 1.
2. JIKA USER MEMILIH LEBIH DARI 1 STORYBOARD: HANYA STORYBOARD TERAKHIR yang boleh dan WAJIB memiliki CTA Final. SEMUA STORYBOARD SEBELUM TERAKHIR SANGAT DILARANG memiliki CTA, ajakan membeli, atau kalimat penutup.
ATURAN MUTLAK KONSISTENSI KARAKTER:
Jika user memilih lebih dari 1 storyboard, Karakter (Talent) UTAMA harus SAMA PERSIS dan KONSISTEN di SEMUA storyboard (Wajah, Rambut, Pakaian, Bentuk tubuh, Warna kulit, Etnis, Usia).
Sediakan JSON format dengan skema persis berisi array dari storyboard (sebanyak storyboardCount yang diminta) dengan properti: title, styleExplanation, scenes (array of {number, name, visual, action, overlay, voiceover, timeRange}), compositeImagePrompt, imageToVideoPrompt.`;
...
- Jika total storyboard > 1, SANGAT DILARANG menaruh CTA di storyboard sebelum terakhir. CTA HANYA boleh ada di storyboard terakhir.
- Jangan gunakan kata pemutus seperti "bersambung", "lanjut part berikutnya" dll.
- Semua storyboard harus membentuk satu kesatuan alur cerita berkelanjutan.
...
- TOTAL SEMUA kata VO yang terisi di satu storyboard WAJIB BERKISAR ANTARA MINIMAL 15 KATA DAN MAKSIMAL 20 KATA.`;
        ...
        // UPDATE MODEL: Gemini 3.5 Flash untuk Text Generation
        const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent?key=${apiKey}`;
        const response = await fetch(apiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
...
async function generateActiveStoryboardImage(index) {
    ...
    const colorInstruction = extractedProductColor !== 'neutral' ? `\n# COLOR & THEME CONSISTENCY\nWarna dominan produk yang di-upload adalah ${extractedProductColor}. Gunakan warna ini sebagai accent color utama untuk border, badge scene number, teks overlay, dan elemen desain lainnya di seluruh storyboard.\n` : '';
    ...
    try {
        if (imageModel === 'imagen') {
            ...
            // UPDATE MODEL: Nano Banana 2 (gemini-3.1-flash-image)
            ...
            const bananaUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-image:generateContent?key=${apiKey}`;
    ...
    if (index === 0 && !characterReferenceImage) {
        ...
        if (storyboardCount > 1) {
            ...
            const refPreview = document.getElementById('characterRefPreview');
            const lockPanel = document.getElementById('characterLockPanel');
            const lockSubtitle = document.getElementById('characterLockSubtitle');
            if (refPreview) refPreview.src = generatedUrl;
            if (lockPanel) {
                lockPanel.classList.remove('hidden');
                if (lockSubtitle) lockSubtitle.innerText = "Wajah dari Part 1 akan dipakai di Part berikutnya";
            }
...
function renderSelectedStoryboard() {
    ...
    const titleEl = document.getElementById('outStoryboardTitle');
    if (titleEl) titleEl.innerText = storyboard.title;
    const styleEl = document.getElementById('outStyleExplanation');
    if (styleEl) styleEl.innerText = storyboard.styleExplanation;
    const engineEl = document.getElementById('outEngineUsed');
    if (engineEl) engineEl.innerText = engine === 'imagen' ? 'Nano Banana Pro' : 'Nano Banana 2';
    const ratioEl = document.getElementById('outRatio');
    if (ratioEl) ratioEl.innerText = ratio;
    const durationEl = document.getElementById('outDuration');
    if (durationEl) durationEl.innerText = `${totalSeconds} Detik`;
    ...
    const vibeEl = document.getElementById('outVibeBadge');
    if (vibeEl) vibeEl.innerText = styleMap[document.getElementById('adStyle').value] || "UGC Ads Vibe";
    ...
    if (storyboard.imageUrl) {
        ...
        if (imageDisplay) { imageDisplay.src = storyboard.imageUrl; imageDisplay.classList.remove('hidden'); }
        const wrapper = document.getElementById('imageWrapper');
        if (wrapper) wrapper.classList.remove('hidden');
        if (brandTag) brandTag.classList.remove('hidden');
        const toolbar = document.getElementById('imageControlToolbar');
        if (toolbar) toolbar.classList.remove('hidden');
        if (visualStatus) { visualStatus.innerText = "Generated"; visualStatus.className = "text-[10px] bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded border border-emerald-500/40"; }
        setTimeout(() => { currentZoom = 'fit'; applyZoom(); }, 50);
    } else {
        if (placeholderContent) placeholderContent.classList.remove('hidden');
        if (loaderContent) loaderContent.classList.add('hidden');
        if (imageDisplay) imageDisplay.classList.add('hidden');
        const wrapper = document.getElementById('imageWrapper');
        if (wrapper) wrapper.classList.add('hidden');
        if (brandTag) brandTag.classList.add('hidden');
        const toolbar = document.getElementById('imageControlToolbar');
        if (toolbar) toolbar.classList.add('hidden');
        if (visualStatus) { visualStatus.innerText = "Belum Digenerate"; visualStatus.className = "text-[10px] bg-zinc-800 text-zinc-400 px-2 py-0.5 rounded border border-zinc-700"; }
    }
    ...
    if (characterReferenceImage) {
        ...
        if (refPreview) refPreview.src = characterReferenceImage.src;
    }
    ...
    const playAllBtn = document.getElementById('playAllAudioBtn');
    const downloadVoBtn = document.getElementById('downloadVoBtn');
    if (playAllBtn) playAllBtn.disabled = false;
    if (downloadVoBtn) downloadVoBtn.disabled = false;
    
    const loader = document.getElementById('workflowLoader');
    const result = document.getElementById('storyboardResult');
...
function toggleDownloadDropdown() { const dd = document.getElementById('downloadDropdown'); if(dd) dd.classList.toggle('hidden'); }
...
function downloadVisual(format) {
    ...
    const base64Url = storyboard.imageUrl; 
    const dd = document.getElementById('downloadDropdown'); if(dd) dd.classList.add('hidden');
...
async function playSingleVoiceover(sceneIndex, buttonElement) {
    ...
    try {
        ...
        if (!audioData) {
            const payload = { contents: [{ parts: [{ text: `Katakan dengan gaya promosi iklan yang sangat persuasif, ekspresif, dan asik sesuai gaya iklan terpilih: ${scene.voiceover}` }] }], generationConfig: { responseModalities: ["AUDIO"], speechConfig: { voiceConfig: { prebuiltVoiceConfig: { voiceName: voice } } } }, model: "gemini-3.1-flash-tts-preview" };
            const apiKey = getNextApiKey();
            if (!apiKey) throw new Error("API Key tidak tersedia.");
            // UPDATE MODEL: Gemini 3.1 Flash TTS
...
async function downloadWholeVO() {
    ...
    try {
        for (let i = 0; i < scenes.length; i++) {
            ...
            if (!base64Data) {
                const payload = { contents: [{ parts: [{ text: `Katakan dengan nada promosi iklan yang asik: ${scene.voiceover}` }] }], generationConfig: { responseModalities: ["AUDIO"], bgType: "RAW", speechConfig: { voiceConfig: { prebuiltVoiceConfig: { voiceName: voice } } } }, model: "gemini-3.1-flash-tts-preview" };
                const apiKey = getNextApiKey();
                if (!apiKey) throw new Error("API Key tidak tersedia.");
                const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-tts-preview:generateContent?key=${apiKey}`;
                const response = await fetch(apiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
...
async function playFullVoiceover() {
    ...
    for (let i = 0; i < scenes.length; i++) {
        if (!isPlayingAudioSequence) break; const scene = scenes[i]; const rows = document.getElementById('storyboardTableBody').children; const row = rows[i]; if(row) row.classList.add('bg-amber-400/10');
        try {
            await new Promise(async (resolve, reject) => {
                if (!scene.voiceover || scene.voiceover.trim() === "") { setTimeout(() => { if(row) row.classList.remove('bg-amber-400/10'); resolve(); }, 500); return; }
                let audioData = scene.audioBase64; let sampleRate = scene.audioSampleRate || 24000;
                if (!audioData) {
                    const payload = { contents: [{ parts: [{ text: `Katakan dengan nada promosi iklan yang asik: ${scene.voiceover}` }] }], generationConfig: { responseModalities: ["AUDIO"], speechConfig: { voiceConfig: { prebuiltVoiceConfig: { voiceName: voice } } } }, model: "gemini-3.1-flash-tts-preview" };
                    const apiKey = getNextApiKey();
                    if (!apiKey) throw new Error("API Key tidak tersedia.");
                    const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-tts-preview:generateContent?key=${apiKey}`;
                    const response = await fetch(apiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
                ...
                if (audioData) {
                    const pcmData = base64ToArrayBuffer(audioData); const pcm16 = new Int16Array(pcmData); const wavBlob = pcmToWav(pcm16, sampleRate);
                    const audioUrl = URL.createObjectURL(wavBlob); const audio = new Audio(audioUrl); currentAudioSource = audio;
                    audio.onended = () => { if(row) row.classList.remove('bg-amber-400/10'); currentAudioSource = null; resolve(); };
                    audio.onerror = () => { if(row) row.classList.remove('bg-amber-400/10'); currentAudioSource = null; reject(new Error("Audio error")); };
                    await audio.play();
                } else resolve();
            });
        } catch (e) { console.error("Sequence interrupted:", e); if(row) row.classList.remove('bg-amber-400/10'); }
    }
    isPlayingAudioSequence = false; updatePlayAllButtonState(false);
}
